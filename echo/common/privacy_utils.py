...
def get_data_loaders(
    ...
    dataset = SiameseUSDataset(
        phase=phase,
        transform=transform,
        latents_csv=latents_csv,
        training_latents_base_path=training_latents_base_path,
        in_memory=False,
...
def train(
    net, training_loader, n_samples, batch_size, criterion, optimizer, epoch, n_epochs
):
    net.train()
    running_loss = 0.0
    ...
    for i, batch in enumerate(training_loader):
        ...
        print(
            "Epoch [%d/%d], Iteration [%d/%d], Loss: %.4f"
            % (
                epoch + 1,
                n_epochs,
                i + 1,
...
def apply_threshold(input_tensor, threshold):
    output = np.where(
        ...
        torch.ones(len(input_tensor)),
        torch.zeros(len(input_tensor)),
    )
    return output
...
def plot_loss_curves(loss_dict, path, experiment_description):
    plt.figure()
    plt.plot(
        range(1, len(loss_dict["training"]) + 1),
        loss_dict["training"],
        label="Training Loss",
    )
    plt.plot(
        range(1, len(loss_dict["validation"]) + 1),
...
def save_roc_metrics_to_file(
    fp_rates, tp_rates, thresholds, path, experiment_description
):
...
def save_loss_curves(loss_dict, path, experiment_description):
    ...
    for i in range(len(loss_dict["training"])):
        f.write(
            str(loss_dict["training"][i])
            + "\t"
            + str(loss_dict["validation"][i])
            + "\n"
        )
    f.close()

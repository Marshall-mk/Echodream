...
from echo.common.datasets import SimaseUSVideoDataset
from echo.privacy.shared import SiameseNetwork


"""
This script is used to apply the privacy filter to a set of synthetic latents.
Example usage:
python echo/privacy/apply.py \
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--synthetic", type=str, help="Path to the synthetic latents folder."
    )
    parser.add_argument(
        "--reference", type=str, help="Path to the real latents folder."
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=95,
        help="Cutoff percentile for privacy threshold.",
    )
    ...
    )  # should be -1 to 1 due to way we trained the model
    ds_train = SimaseUSVideoDataset(
        phase="training",
        transform=normalization,
        latents_csv=training_latents_csv,
        training_latents_base_path=training_latents_basepath,
        in_memory=True,
    )
    ds_test = SimaseUSVideoDataset(
        phase="testing",
        transform=normalization,
        latents_csv=training_latents_csv,
        training_latents_base_path=training_latents_basepath,
    ...
    synthetic_images = [
        torch.load(os.path.join(args.synthetic, x)) for x in synthetic_images_paths
    ]
    ...
    net = SiameseNetwork(
        network=config["siamese_architecture"],
        in_channels=config["n_channels"],
        n_features=config["n_features"],
    )
    net.load_state_dict(
        torch.load(
            os.path.join(args.model, os.path.basename(args.model) + "_best_network.pth")
        )
    )

    print("Sanity Check")
    print(
        f"Train - Min: {train_images.min()} - Max: {train_images.max()} - shape: {train_images.size()}"
    )
    print(
        f"Test - Min: {test_images.min()} - Max: {test_images.max()} - shape: {test_images.size()}"
    )
    print(
        f"Train - Min: {synthetic_images.min()} - Max: {synthetic_images.max()} - shape: {synthetic_images.size()}"
    )
    ...
    with torch.no_grad():
        for i in tqdm(
            np.arange(0, len(train_images), bs), "Computing Train Embeddings"
        ):
        ...
        for i in tqdm(
            np.arange(0, len(synthetic_images), bs), "Computing Synthetic Embeddings"
        ):
    ...
    # Plot the results
    fig, ax = plt.subplots()
    nt, bins, patches = ax.hist(
        closest_train, 100, density=True, label="Train-Val", alpha=0.5, color="blue"
    )
    ns, bins, patches = ax.hist(
        closest_test, 100, density=True, label="Train-Synth", alpha=0.5, color="orange"
    )
    ...
    print(
        f"Number of non private synthetic images: {sum(is_public)} - memorization rate: {1 - is_public.float().mean()}"
    )
    ...
    for clos_idx, tau_val, path in zip(
        closest_loc, closest_test, synthetic_images_paths
    ):
    ...
    res.to_csv(
        os.path.join(
            os.path.dirname(private_latents_output_path), "privacy_scores.csv"
        ),
        index=False,
    )

...
from echo.common import privacy_utils as Utils
...
class AgentSiameseNetwork:
    def __init__(self, config):
        ...
        self.SAVINGS_PATH = (
            "./experiments/" + self.config["experiment_description"] + "/"
        )
        ...
        if self.config["resumption"] is True:
            self.checkpoint = torch.load(
                "./archive/"
                + self.config["previous_experiment"]
                + "/"
                + self.config["previous_experiment"]
                + "_checkpoint"
                + self.load_suffix
        ...
        self.training_loader = Utils.get_data_loaders(
            phase="training",
        ...
        self.validation_loader = Utils.get_data_loaders(
            phase="validation",
            data_handling=self.data_handling,
            n_channels=self.n_channels,
        ...
        self.test_loader = Utils.get_data_loaders(
            phase="testing",
    ...
    def training_validation(self):
        ...
        for epoch in range(self.start_epoch, self.max_epochs):
            ...
            validation_loss = Utils.validate(
                self.net,
            ...
            Utils.save_loss_curves(
                self.loss_dict, self.SAVINGS_PATH, self.config["experiment_description"]
            )
    ...
    def testing_evaluation(self):
        ...
        auc_mean, confidence_lower, confidence_upper = Utils.bootstrap(
            10000,
            y_true,
            y_pred,
            self.SAVINGS_PATH,
        ...
        Utils.plot_roc_curve(
            fp_rates, tp_rates, self.SAVINGS_PATH, self.config["experiment_description"]
        )

        # Save all the results to files!
        Utils.save_labels_predictions(
            y_true,
            y_pred,
            y_pred_thresh,
            self.SAVINGS_PATH,
        ...
        Utils.save_roc_metrics_to_file(
            fp_rates,
            tp_rates,
            thresholds,
            self.SAVINGS_PATH,
        ...
        print(
            ...
            + str(confidence_lower)
            + " - "
            + str(confidence_upper)
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--config_path", default="./", help="the path where the config files are stored"
    )
    parser.add_argument(
        "--config",
        default="config.json",
    ...
    os.makedirs(
        os.path.join("./experiments/", config["experiment_description"]), exist_ok=True
    )

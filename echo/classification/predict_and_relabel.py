...
def load_model(backbone, checkpoint_path, device):
    ...
    if backbone == "resnet50" or backbone == "resnet43":
        model = get_video_classifier(num_classes=num_classes, backbone=backbone, pool_type=pool_type)
    ...
    #     # This is likely a VideoResNet model
    #     # Determine backbone from state_dict shape
    #     if any("layer4.2" in key for key in state_dict.keys()):
    #         backbone = "resnet50"  # Has bottleneck layers
    #     elif any("layer4.1" in key for key in state_dict.keys()) and not any("layer4.2" in key for key in state_dict.keys()):
    #         backbone = "resnet34"  # No bottleneck layers
    #     else:
    #         backbone = "resnet18"  # Default fallback
    ...
    #     print(f"Detected backbone: {backbone} with {pool_type} pooling")
    #     model = get_video_classifier(num_classes=num_classes, backbone=backbone, pool_type=pool_type)
        
    # elif any("model.stem" in key for key in state_dict.keys()):
    #     # This is likely a 3D model like r3d_18 or r2plus1d_18
    #     if any("model.layer3.0.conv2.1" in key for key in state_dict.keys()):
...
def predict_videos(model, dataloader, device):
    """
    ...
    Returns:
        Tuple of (all_predictions, all_video_ids)
    """
    all_predictions = []
    all_video_ids = []
    ...
    try:
        ...
        if hasattr(dataloader.dataset, 'df') and 'FileName' in dataloader.dataset.df.columns:
        ...
        elif hasattr(dataloader.dataset, 'df') and 'video_id' in dataloader.dataset.df.columns:
...
def update_csv_with_predictions(csv_path, video_ids, predictions, classes, output_csv=None):
    """
    Update the input CSV with predicted class labels
...
def parse_args():
    parser = argparse.ArgumentParser(description="Predict video classes using a trained model")
    ...
    parser.add_argument("--frame-sampling", type=str, default="uniform",
    ...
    parser.add_argument("--selected-classes", type=str, nargs="+", default=None,
...
def main():
    ...
    splits_to_process = ["TRAIN", "VAL", "TEST"] if "ALL" in args.splits else args.splits
    ...
    for split in splits_to_process:
        ...
        dataloader, temp_csv_path = create_prediction_dataloader(dataloader_config, split)
        ...
        predictions, video_ids = predict_videos(model, dataloader, device)
        ...
        updated_csv = update_csv_with_predictions(
            temp_csv_path, video_ids, predictions, classes, output_csv
        )

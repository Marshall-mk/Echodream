...
def merge_csv_files(
    ...
    if rename_columns:
        ...
        missing_columns = [
            col for col in rename_columns.keys() if col not in merged_df.columns
        ]
        if missing_columns:
            print(
                f"Warning: The following columns to rename don't exist in the merged DataFrame: {missing_columns}"
            )
            # Only rename columns that exist
            rename_dict = {
                k: v for k, v in rename_columns.items() if k in merged_df.columns
            }
        else:
...
def main():
    ...
    parser.add_argument(
        "--output", type=str, required=True, help="Path to save the merged CSV file"
    )
    parser.add_argument(
        "--no-ignore-index",
        action="store_false",
    ...
    parser.add_argument(
        "--sort", action="store_true", help="Sort the columns in the merged file"
    )
    parser.add_argument(
        "--join",
        type=str,
    ...
    if args.rename_columns:
        try:
            ...
            print(
                "Error: Column renaming arguments must be in format 'old_name:new_name'"
            )
            return

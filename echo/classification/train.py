...
def train_epoch(model, dataloader, criterion, optimizer, accelerator, epoch):
    ...
    for batch_idx, (inputs, targets) in enumerate(dataloader):
        if debug==True and batch_idx==0 and accelerator.is_local_main_process:
            ...
            for i, video in enumerate(inputs):
                video_path = debug_dir + f"class_{targets[i].item()}_batch_{batch_idx}_video_{i}.mp4"
    ...
    loss_tensor = torch.tensor(running_loss / len(dataloader), device=accelerator.device)
    acc_tensor = torch.tensor(100. * correct / total, device=accelerator.device)
...
def validate(model, dataloader, criterion, accelerator):
    ...
    if accelerator.is_local_main_process:
        print(f'Validation Loss: {loss_tensor.item():.4f} | Validation Acc: {acc_tensor.item():.2f}%')
...
def parse_args():
    ...
    parser.add_argument('--synthetic-csv-path', type=str, default=None, 
                        help='Path to CSV file with synthetic video metadata')
    parser.add_argument('--synthetic-data-dir', type=str, default=None, 
                        help='Directory containing synthetic video frames')
    parser.add_argument('--use-synthetic-for', type=str, nargs='+', default=[], 
    ...
    # Output parameters
    parser.add_argument('--output-dir', type=str, default='./experiments/classification/real', 
...
def main():
    ...
    criterion = nn.CrossEntropyLoss(weight=dataloaders['train'].dataset.class_weights.to(accelerator.device))
    ...
    checkpoint = torch.load(output_dir / "best_model.pth", map_location=accelerator.device)

...
class VideoDataset(Dataset):
    ...
    def __init__(
        ...
        self.use_synthetic = synthetic_csv_path is not None and synthetic_data_dir is not None
        ...
        self.use_synthetic_for_current_split = self.use_synthetic and split in self.use_synthetic_for_split
        ...
        if self.use_synthetic and self.use_synthetic_for_current_split:
            ...
            synthetic_df = synthetic_df[synthetic_df['split'] == split].reset_index(drop=True)
    ...
    def __getitem__(self, idx):
        ...
        base_dir = self.synthetic_data_dir if self.use_synthetic_for_current_split else self.data_dir
    ...
    def _sample_frames(self, video_dir, num_frames):
        ...
        """
        available_frames = sorted([f for f in os.listdir(video_dir) if f.endswith(('.jpg', '.png'))])
        ...
        if num_frames < self.frames_per_clip:
            ...
            frame_indices = np.linspace(0, num_frames - 1, self.frames_per_clip, dtype=int)
        else:
            if self.frame_sampling == 'uniform':
                ...
                if self.split == 'TRAIN':
                    frame_indices = sorted(np.random.choice(
                        num_frames, self.frames_per_clip, replace=num_frames < self.frames_per_clip
                    ))
                else:
                    # For val/test, use uniform sampling for reproducibility
                    frame_indices = np.linspace(0, num_frames - 1, self.frames_per_clip, dtype=int)
            else:
                raise ValueError(f"Unsupported frame sampling method: {self.frame_sampling}")
...
def get_video_transforms(split, input_size=112, mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225]):
    """
...
def create_video_dataloaders(
    ...
    pin_memory = config.get('pin_memory', False)  # Default to False to avoid CUDA errors

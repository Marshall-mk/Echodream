...
from echosyn.common import padf, unpadf, instantiate_from_config
...
def log_validation(
    config, unet, vae, scheduler, accelerator, weight_dtype, epoch, val_dataset
):
    ...
    for tracker in accelerator.trackers:
        if tracker.name == "wandb":
            tracker.log({"validation": wandb.Image(images)})
    ...
    for epoch in range(first_epoch, config.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                model_pred = unet(sample=noisy_latents, timestep=timesteps).sample

...
from echo.common import (
    pad_reshape, unpad_reshape, padf, unpadf, load_model, 
    save_as_mp4, save_as_gif, save_as_avi, save_as_img,
    parse_formats, FlowMatchingScheduler
)
...
def parse_args():
    ...
    parser.add_argument("--conditioning_type", type=str, default="class_id",
    ...
    parser.add_argument("--lvef", type=float, default=None,
    ...
    # Conditioning frame
    parser.add_argument("--conditioning_frame", type=str, default=None,
                        help="Specific conditioning frame to use (filename). If not specified, a random one will be chosen.")

    # Sampling parameters
    parser.add_argument("--sampling_mode", type=str, default="diffusion",
    ...
    # Output parameters
    parser.add_argument("--format", type=parse_formats, default=["mp4"],
...
def load_conditioning_frames(args):
    ...
    if file_ext not in ["pt", "jpg", "png"]:
        raise ValueError(f"Unsupported file extension: {file_ext}. Must be pt, jpg, or png.")
    ...
    if args.conditioning_frame and args.conditioning_frame in files:
        ...
        if args.conditioning_frame:
            print(f"Warning: Specified conditioning frame {args.conditioning_frame} not found. Using random frame.")
...
def generate_video(args, config, unet, vae, scheduler, device, frame_path, file_ext):
    ...
    args.frames = int(np.ceil(args.frames / 32) * 32)  # Make sure frames are a multiple of 32
    if args.frames > T:
        OT = T // 2  # overlap
    ...
    if file_ext == "pt":
        ...
        with torch.no_grad():
            latent_cond_images = vae.encode(image_tensor).latent_dist.sample() * vae.config.scaling_factor
    ...
    if config.unet._class_name == "UNetSpatioTemporalConditionModel":
        dummy_added_time_ids = torch.zeros(
            (B * TR, config.unet.addition_time_embed_dim), device=device, dtype=torch.float32
        )
    ...
    with torch.no_grad():
        for t in progress_bar:
            ...
            if use_guidance and args.sampling_mode == "diffusion":
                ...
                inputs = torch.cat([
                    latent_model_input[:, r*(T-OT):r*(T-OT)+T] for r in range(TR)
                ], dim=0)
                ...
                for r in range(TR):
                    ...
                    uncond_chunk = outputs_uncond[r] if r == 0 else outputs_uncond[r][:, OT:]
                    guided_chunk = uncond_chunk + args.guidance_scale * (cond_chunk - uncond_chunk)
...
def main():
    ...
    # Generate video
    video = generate_video(args, config, unet, vae, scheduler, device, frame_path, file_ext)

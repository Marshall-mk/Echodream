...
--model /nfs/usrhome/khmuhammad/Echonet/models/vae  
--input /nfs/usrhome/khmuhammad/Echodream/datasets/CardiacNet   
--output /nfs/usrhome/khmuhammad/Echodream/data/latents/cardiacnet  
--gray_scale
"""
...
class VideoDataset(torch.utils.data.Dataset):
    def __init__(self, folder):
        self.folder = folder
        self.videos = sorted(glob(os.path.join(folder, "*.mp4")))
        self.valid_videos = self.videos.copy()  # Keep track of valid videos
        self.skipped_videos = []  # Keep track of skipped videos
    ...
    def __getitem__(self, idx):
        try:
            video_path = self.valid_videos[idx]
            video, fps = loadvideo(video_path, return_fps=True)
            return video, video_path, fps
        except Exception as e:
            print(f"Error loading video {self.valid_videos[idx]}: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", type=str, required=True, help="Path to input folder"
    )
    parser.add_argument(
        "-o", "--output", type=str, required=True, help="Path to output folder"
    )
    parser.add_argument(
        "-g",
        "--gray_scale",
        action="store_true",
        help="Convert to gray scale",
        default=False,
    )
    parser.add_argument(
        "-f",
    ...
    for video, vpath, fps in tqdm(dl):
        try:
            ...
            opath = vpath.replace(
                ...
                1:
            ]  # retrieve relative path to input folder, similar to basename but keeps the folders
            opath = opath.replace(".mp4", f".pt")  # change extension
            ...
            if needs_df_update:
                ...
                df.loc[
                    df["FileName"] == fname,
                    ["FileName", "FrameHeight", "FrameWidth", "FPS", "NumberOfFrames"],
                ] = [fname, 112, 112, fps, len(video)]
        except Exception as e:

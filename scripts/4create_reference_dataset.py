...
python scripts/4create_reference_dataset.py --dataset /nfs/usrhome/khmuhammad/Echodream/datasets/CardiacNet/ --output /nfs/usrhome/khmuhammad/Echodream/data/reference/cardiacnet --frames 16
"""

if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument(
        "--dataset", type=str, required=True, help="Path to the dataset directory"
    )
    ...
    parser.add_argument(
        ...
        type=float,
        default=32,
        help="Target FPS of the video before frame extraction. Should match the diffusion model FPS.",
    )
    ...
    assert os.path.exists(video_path), (
        f"Could not find Videos directory at {video_path}"
    )
    ...
    if extracted_videos < target_count:
        print(
            f"WARNING: only saved {extracted_videos} videos, which is less than the target count of {target_count}."
        )

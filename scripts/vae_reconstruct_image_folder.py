...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Encode and decode images using a trained VAE model."
    )
    parser.add_argument(
        "-m", "--model", type=str, help="Path to the trained VAE model."
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=128,
        help="Batch size to use for encoding and decoding.",
    )
    ...
    dataloader = DataLoader(
        ...
        shuffle=False,
        num_workers=max(args.batch_size, 32),
    )

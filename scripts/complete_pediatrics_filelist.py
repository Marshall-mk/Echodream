...
if __name__ == "__main__":
    ...
    metadata.to_csv(
        os.path.join(args.dataset, "FileList_ORIGINAL.csv"), index=False
    )  # backup
    ...
    metadata["Split"] = metadata["Fold"].apply(
        lambda x: "TRAIN" if x in range(8) else "VAL" if x == 8 else "TEST"
    )
    ...
    for i, row in tqdm(metadata.iterrows(), total=len(metadata)):
        ...
        metadata.loc[i, ["FrameHeight", "FrameWidth", "FPS", "NumberOfFrames"]] = [
            height,
            width,
            fps,
            nframes,
